...
partial model PartialPumpParallel_m "Partial model for pump parallel"
 ...
 // Pump parameters
  ...
  parameter Real threshold(min = 0.01*m_flow_nominal/num) = 0.05*m_flow_nominal/num
    "Hysteresis threshold";
  Modelica.Blocks.Interfaces.RealInput u[num](
    each final unit="kg/s",
    each max=1.1*m_flow_nominal,
    each min=0)
    "required mass flow rate input signal for the flow machine"
  ...
  replaceable Buildings.Fluid.Movers.BaseClasses.PartialFlowMachine pum[num](each
...
equation
  ...
  connect(pum.P, mul1.u1)
    annotation (Line(points={{11,9},{26,9},{26,46},{54,46}}, color={0,0,127}));
  connect(mul1.y, P)
    annotation (Line(points={{78,40},{110,40}}, color={0,0,127}));
  connect(swi.y, mul1.u2) annotation (Line(points={{-26,-30},{28,-30},{28,20},{
          50,20},{50,34},{54,34}}, color={0,0,127}));
