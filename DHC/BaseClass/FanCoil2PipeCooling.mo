...
model FanCoil2PipeCooling "Model of a sensible only two-pipe fan coil unit for cooling,
  computing a required chilled water mass flow rate"
  extends DHC.BaseClass.PartialTerminalUnit(
  ...
  Buildings.Fluid.HeatExchangers.WetCoilEffectivenessNTU hexWetNtu(
    ...
    final dp1_nominal=dpClgNom,
    final dp2_nominal=0,
    use_Q_flow_nominal=true,
    final Q_flow_nominal=1.2*QCoo_flow_nominal,
    ...
    final w_a2_nominal=w_aLoaCoo_nominal) "Cooling coil"
  ...
  Buildings.Fluid.Sources.Boundary_pT sinAir(
    ...
    use_T_in=false,
    nPorts=1) "Sink for supply air" annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={-110,0})));
  Buildings.Fluid.Sources.Boundary_pT retAir(
    redeclare package Medium = Medium2,
    use_T_in=true,
    nPorts=1) "Source for return air" annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=0,
        origin={112,0})));
  ...
  Modelica.Blocks.Sources.RealExpression exp(each y=if u == Md then 1 else 0)
    annotation (Placement(transformation(extent={{86,76},{106,96}})));
  Modelica.Blocks.Math.Product pro_C
...
equation
  ...
  connect(mulQReqCoo_flow.y,TLoaODE.QReq_flow)
    annotation (Line(points={{-158,40},{-100,40},{-100,40},{-12,40}},color={0,0,127}));
  connect(Q_flowCoo.y,mulQActCoo_flow.u)
    annotation (Line(points={{141,200},{158,200}},color={0,0,127}));
  connect(TSetCoo,TLoaODE.TSet)
    annotation (Line(points={{-220,160},{-120,160},{-120,48},{-12,48}},color={0,0,127}));
  connect(mulQReqCoo_flow.y,gaiHeaFlo.u)
    annotation (Line(points={{-158,40},{-100,40},{-100,220},{-90,220}},color={0,0,127}));
  ...
  connect(one.y, swi.u3) annotation (Line(points={{12,140},{18,140},{18,172},{24,
          172}}, color={0,0,127}));
  connect(gaiMasFlo.y, pro_C.u2) annotation (Line(points={{62,220},{82,220},{82,
          74},{116,74}},                     color={0,0,127}));
  connect(exp.y,pro_C. u1) annotation (Line(points={{107,86},{116,86}},
                    color={0,0,127}));
  connect(pro_C.y, mulMasFloReqChiWat.u)
    annotation (Line(points={{139,80},{158,80}}, color={0,0,127}));
  annotation (
    Documentation(
      info="<html>
<p>
