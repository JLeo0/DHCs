...
protected
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume vol_b(
    redeclare final package Medium = Medium,
    final m_flow_nominal=m_flow_nominal,
    ...
    final mSenFac=if rho_default > 500 then 1 else 10) if have_pipCap "Control volume connected to port_b.
    Represents equivalent pipe wall thermal capacity."
    annotation (Placement(transformation(extent={{70,20},{90,40}})));
  Buildings.Fluid.FixedResistances.LosslessPipe noMixPip_b(
    redeclare final package Medium = Medium,
    final m_flow_nominal=m_flow_nominal,
    final m_flow_small=m_flow_small,
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume vol_a(
    redeclare final package Medium = Medium,
    final m_flow_nominal=m_flow_nominal,
    final V=if rho_default > 500 then VEqu else VEqu/1000,
    final nPorts=2,
    final T_start=T_start_out,
    final energyDynamics=Modelica.Fluid.Types.Dynamics.FixedInitial,
    final mSenFac=if rho_default > 500 then 1 else 10) if have_pipCap_a "Control volume connected to port_a.
    Represents equivalent pipe wall thermal capacity."
    annotation (Placement(transformation(extent={{-90,20},{-70,40}})));
  Buildings.Fluid.FixedResistances.LosslessPipe noMixPip_a(
    redeclare final package Medium = Medium,
    final m_flow_nominal=m_flow_nominal,
    final m_flow_small=m_flow_small,
    final allowFlowReversal=allowFlowReversal) if not have_pipCap_a "Lossless pipe for connecting the outlet port when have_pipCap=false
...
equation
  ...
  connect(port_a, vol_a.ports[1])
    annotation (Line(points={{-100,0},{-81,0},{-81,20}}, color={0,127,255}));
  connect(vol_a.ports[2], heaLos_a.port_b)
    annotation (Line(points={{-79,20},{-79,0},{-60,0}}, color={0,127,255}));
